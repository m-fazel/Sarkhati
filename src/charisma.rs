...
pub struct CharismaConfig {
    ...
    pub orders: Vec<CharismaOrderData>,
...
fn default_order_url() -> String {
    "https://charisma.exirbroker.com/api/v1/order".to_string()
}
...
pub async fn send_order(
    config: &CharismaConfig,
    order: &CharismaOrderData,
    test_mode: bool,
    curl_only: bool,
    ...
    println!("[Charisma] Generated X-App-N: {}", x_app_n);

    let order_json = serde_json::to_string(order)?;

    // Print curl command in test mode
    if test_mode {
        println!("[Charisma] Equivalent curl command:");
        println!(r#"curl '{}' \
  --compressed \
  -X POST \
  -H 'User-Agent: {}' \
  ...
  --data-raw '{}'"#,
    ...
    println!("[Charisma] Sending order JSON: {}", order_json);
    ...
    println!("[Charisma] Order response status: {}", status);
    println!("[Charisma] Order response body: {}", decoded_text);
...
pub async fn run_calibration(
    ...
    calibration::run_calibration("[Charisma]", calibration, rate_limiter, || {
