...
fn default_order_url() -> String {
    "https://api.daybroker.ir/Web/V1/Order/Post".to_string()
}
...
pub async fn send_order(
    ...
    if test_mode {
        println!("[Day] Equivalent curl command:");
        println!(r#"curl '{}' \
  --compressed \
  -X POST \
  -H 'User-Agent: {}' \
  ...
  --data-raw '{}'"#,
    ...
    println!("[Day] Sending order JSON: {}", order_json);
    ...
    println!("[Day] Order response status: {}", status);
    println!("[Day] Order response body: {}", decoded_text);
...
pub async fn run_calibration(
    ...
    calibration::run_calibration("[Day]", calibration, rate_limiter, || {
