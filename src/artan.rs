...
fn default_order_url() -> String {
    "https://artan.exirbroker.com/api/v1/order".to_string()
}
...
pub async fn send_order(
    ...
    println!("[Artan] Generated X-App-N: {}", x_app_n);

    let order_json = serde_json::to_string(order)?;

    // Print curl command in test mode
    if test_mode {
        println!("[Artan] Equivalent curl command:");
        println!(r#"curl '{}' \
  --compressed \
  -X POST \
  -H 'User-Agent: {}' \
  ...
  --data-raw '{}'"#,
    ...
    println!("[Artan] Sending order JSON: {}", order_json);
    ...
    println!("[Artan] Order response status: {}", status);
    println!("[Artan] Order response body: {}", decoded_text);
...
pub async fn run_calibration(
    ...
    calibration::run_calibration("[Artan]", calibration, rate_limiter, || {
