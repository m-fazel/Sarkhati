...
pub struct NibbourseConfig {
    ...
    pub orders: Vec<NibbourseOrderData>,
...
fn default_order_url() -> String {
    "https://nibbourse.exirbroker.com/api/v1/order".to_string()
}
...
pub async fn send_order(
    config: &NibbourseConfig,
    order: &NibbourseOrderData,
    test_mode: bool,
    curl_only: bool,
    ...
    println!("[Nibbourse] Generated X-App-N: {}", x_app_n);

    let order_json = serde_json::to_string(order)?;

    // Print curl command in test mode
    if test_mode {
        println!("[Nibbourse] Equivalent curl command:");
        println!(r#"curl '{}' \
  --compressed \
  -X POST \
  -H 'User-Agent: {}' \
  ...
  --data-raw '{}'"#,
    ...
    println!("[Nibbourse] Sending order JSON: {}", order_json);
    ...
    println!("[Nibbourse] Order response status: {}", status);
    println!("[Nibbourse] Order response body: {}", decoded_text);
...
pub async fn run_calibration(
    ...
    calibration::run_calibration("[Nibbourse]", calibration, rate_limiter, || {
